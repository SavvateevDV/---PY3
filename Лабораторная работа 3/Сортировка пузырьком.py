...
def sort(container: Sequence[int]) -> Sequence[int]:
    ...
    f = len(container)  # переменная длины массива
    for a in range(f):  # задается количество циклов
        for i in range(0, len(container)-1):  # цикл сортировки
            if container[i] > container[i+1]:
                container[i], container[i+1] = container[i+1], container[i]  # перестановка элементов массива
    return container
# пустая строка